(function () {
  ...
  function injectStyles() {
    ...
    const css = `
      ...
      .${CONFIG.AREA_ID}-content {
        ...
        width: 100%;
        position: relative; /* relative positioning for the 'read more' button */
        transition: max-height 0.3s ease-out; /* Add transition for smooth expansion */
      }
      .${CONFIG.AREA_ID}-content.collapsed {
        max-height: 4rem; /* Initial collapsed height */
        overflow: hidden;
      }
      .${CONFIG.AREA_ID}-content.expanded {
        max-height: 1000px; /* Large enough value for expansion */
      }
      .${CONFIG.AREA_ID}-read-more {
        text-align: right;
        font-weight: bold;
        cursor: pointer;
        color: #007bff;
  ...
  function createNotificationItem(rec) {
    ...
    // Check for overflow and add 'read more' button if needed
    // Temporarily append to measure dimensions, then check for overflow
    const tempContainer = document.createElement('div');
    tempContainer.style.visibility = 'hidden';
    document.body.appendChild(tempContainer);
    tempContainer.appendChild(wrapper.cloneNode(true));
    const tempContentDiv = tempContainer.querySelector(`.${CONFIG.AREA_ID}-content`);
    
    // Set initial collapsed class to measure correctly
    tempContentDiv.classList.add('collapsed');

    if (tempContentDiv.scrollHeight > tempContentDiv.clientHeight) {
      contentDiv.classList.add('collapsed');

      const readMoreBtn = document.createElement('div');
      readMoreBtn.className = `${CONFIG.AREA_ID}-read-more`;
