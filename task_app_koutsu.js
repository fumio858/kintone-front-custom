...
(function () {
  ...
  async function initTaskPanel(mountEl, rec) {
    ...
    function normalizeYMD(s){ if(!s) return ''; const ymd=String(s).trim().slice(0,10); return /^\d{4}-\d{2}-\d{2}$/.test(ymd)? ymd.replace(/-/g,''):''; }
    function toYmdNum(s){ const n=normalizeYMD(s); return n?Number(n):NaN; }
    ...
    async function fetchTasks() {
      ...
      try {
        ...
        if ((resp.records||[]).length===0 && /^[0-9]+$/.test(c)) {
          const resp2 = await kintone.api(kUrl('/k/v1/records.json'), 'GET', { app: TASK_APP_ID, query: `${T_CASE_ID} = ${c} order by ${T_DUE} asc` });
    ...
    function row(tRec){
      ...
      div.addEventListener('click', async (e)=>{
        ...
        try{
          ...
          render();
        }catch(e2){ console.error('更新エラー', e2); alert('タスク更新に失敗しました。'); }
      });
      return div;
    }
