(function () {
  ...
  kintone.events.on('app.record.detail.show', function (event) {
    ...
    const setupClickListener = (button) => {
      // ガードを追加：リスナーが既に追加されている場合は何もしない
      if (button.dataset.swalListenerAttached) {
        return;
      }
      button.dataset.swalListenerAttached = 'true'; // フラグを設定
      ...
      button.addEventListener('click', async (e) => {
        ...
        actionElements.forEach((el, index) => {
          ...
          let actionLabel = actionLabelElement?.innerText;
        ...
        Swal.fire({
          ...
          html: `<div style="max-height: 45vh; overflow-y: auto;">` + `<div style="text-align: left; padding: 0 1em; margin-bottom: 1em; text-align: center; background-color: #c7c7c745; color: #000000; padding: 1rem; line-height: 2; letter-spacing: 0.05rem; font-size: 1.3rem; border-color: #FFF; border-width: thick;">${statusInfo}</div>` + actionButtonsHtml + `</div>`,
